...
const Home = () => {
      ...
      <div>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6'>
            {
              products.filter((item) => item.category === "men").slice(0, 8).map((product) => (
                <div
                  key={product.id}  // Use product.id instead of products.id
                  className='my-5 mx-auto border border-gray-200 rounded-lg text-left shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out w-64 h-80'  // Fixed card size
                >
                  <Link to={`${product.name}`} key={product.id}>
                    <div className='overflow-hidden h-48'>  {/* Fixed image container height */}
                      <img
                        src={product.image}
                    ...
                    <div className="p-4">
                      <div className='font-bold text-lg text-gray-800 truncate'>{product.name}</div>  {/* 'truncate' to prevent overflow */}
        ...
        <div className='text-center  place-content-center'>
          ...
          <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6 ml-2'>
            {products
              .filter((item) => item.category === "kid" || item.category === "kids")
              .slice(0, 4)
              .map((product) => (
                <div
                  key={product.id}
                ...
                >
                  <Link to={`/${product.name}`} key={product.id}>
                    <div className='overflow-hidden h-48'> {/* Fixed height for image container */}
                      <img
                        src={product.image}
                        onClick={() => SinglePage(product)}
                        alt="product image"
  ...
  {products
    .filter((item) => item.category === "women" || item.category === "womens")
    .slice(0, 8)
    .map((product) => (
      <div
        key={product.id}
      ...
      >
        <Link to={`/${product.name}`} key={product.id}>
          <div className='overflow-hidden h-48 mb-3'> {/* Fixed image container height */}
            <img
              src={product.image}
              onClick={() => SinglePage(product)}
              alt="product"
